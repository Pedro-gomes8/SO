...
// Defining how long the haircut should last. Currently 0.5s.
...
void *handle_barber()
{
    while(1){
        ...
        if (amountOfCustomers == 0){
            ...
            if (BarberHasSleptOnce){ // If there is no customer and the barber has already slept, he shall close the shop.
            ...
            // If he is awake and there is a customer, he cuts the customer's hair and say goodbye to them. 
            pthread_mutex_unlock(&queueMutex);
            printf("Cutting Hair...\n");
            nanosleep(&haircutTime,NULL);

            // Queue manipulation, the barber grabs the lock to manipulate the queue and the amount of customers in line.
            ...
            // Changing the amount of customers left as he just cut someone's hair.
            ...
            switch (BarberHasSleptOnce) // If barber wasnt the first created thread to be executed, that means there was no need for him to sleep for the first time. In that case, the first time he'd sleep would be whenever the queue would be empty. Essentially, this block means: "If barber hasnt slept up to this point, lie about it and say he has slept once."
            {
            case 0:
...
{   
    ...
    // Queue and 'amountOfCustomers' manipulation, lock is necessary.
    pthread_mutex_lock(&queueMutex);
    while (amountOfCustomers >= numberOfChairs){ // If there's no empty chair:
...
{
    printf("\nIMPORTANTE. Apos alguns testes, eu percebi que esse codigo possui uma falha que pode aparecer, quando for testar que nao consegui solucionar. De alguma forma, apenas uma unica thread (independente dos parametros que esse codigo recebe) 'perde' seu ID. Seu ID passa a ser 0 como se fosse a primeira thread criada, e essa thread que perde o ID acaba no final da queue. Isso pode aparecer nos resultados. Embora uma unica thread possa perder, ou nao, seu lugar na fila, todas as threads que entram na barbearia continuam tendo seu cabelo cortado yay!\n\n");
